...
import { MIN_LENGTH, REQUIRED_ERROR, REQUIRED_TYPE } from '../../utils/Utilities'
...
const Login = () => {
    ...
    const onSubmit = () => {
        ...
        if (isDirty && isValid) {
            dispatch(handleLogin(getValues())).then((data) => {
                if (data) {
                    notifySuccess({ header: "Success", message: "User login successfully." })
                    history.push('/covid')
                }
            })
        }
    ...
    return (
        <React.Fragment>
            <Card className="mx-auto justify-content-center mt-5" style={{ maxWidth: "400px" }}>
